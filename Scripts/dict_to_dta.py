...
def dict_to_dta_list(song_dict: dict, indent=0):
    lines = []
    for song_k,song_v in song_dict.items():
        if isinstance(song_v, dict):
            lines.append(f"{'    '*indent}({song_k}")
            lines.extend(dict_to_dta_list(song_v, indent+1))
...
def main():
    ...
    # print(dict_to_dta_list(big_song_dict["songs"]))
    for line in dict_to_dta_list(big_song_dict["songs"]):
        print(line)
